...
const ContactForm = () => {
  const handleSubmit = (values, {resetForm}) => {
    axios
      .post('http://localhost:3001/usersData', values)
      .then(response => {
  ...
  return (
    <Formik
      ...
      validationSchema={Yup.object({
        type: Yup.string().required('Select the type of enquiry'),
        name: Yup.string().min(2, 'At least 2 characters').required('Required field!'),
        email: Yup.string().email('Incorrect address').required('Required field!'),
        message: Yup.string().min(10, 'At least 10 characters').required('Required field!'),
      })}
